...
class EdocViewer {
  ...
  constructor() {
    // Use an async IIFE to allow async initialization
    (async () => {
      try {
  ...
  private async initializeApp(): Promise<void> {
    ...
    if (action === "upload") {
      ...
      await this.loadFromUrl(this.originalUrl);
    } else {
      this.setupUploadInterface();
    }
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        await this.downloadOriginalFile();
      });
  ...
  private async handleFileSelection(file: File): Promise<void> {
    ...
    try {
      const arrayBuffer = await new Promise<ArrayBuffer>((resolve, reject) => {
        ...
        reader.onerror = () =>
          reject(new Error(i18n.translate("error.fileNotFound")));
      ...
      container.documentFileList.forEach((filename) => {
        ...
        if (fileContent) {
          ...
          downloadButton.addEventListener("click", async () => {
            await this.downloadContainerFile(actualFilename, fileContent);
          });
          ...
          if (fileExtension === "pdf") {
            ...
            viewButton.addEventListener("click", async () => {
              await this.viewFile(actualFilename, fileContent);
            });
          }
   ...
   */
  private async viewFile(filename: string, content: Uint8Array): Promise<void> {
    try {
      const mimeType = this.getMimeTypeFromFilename(filename);
   ...
   */
  private async downloadOriginalFile(): Promise<void> {
    if (!this.fileData) {
    ...
    try {
      await downloadFile(filename, this.fileData, {
